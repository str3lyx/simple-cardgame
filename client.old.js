...
client.on('playerTurn', function (top, err) {
  ...
  if (hasValidCard(top)) {
    ...
    readline.question(
      `Pick your card (or press 9 to pass your turn)\n> `,
      (choice) => {
        if (state != 4) return
        ...
        if (index >= 0) {
          if (isValidCard(cards[index], top)) {
            ...
            client.emit(
              'playerTurn',
              top,
              `${getCardName(cards[index])}'s honer is lower than ${getCardName(
                top
              )}'s.`
            )
        } else if (index == -1) {
          ...
          client.emit('playerTurn', top, 'Invalid Character: Out of range')
        else if (index == -3)
          client.emit('playerTurn', top, 'Invalid Character: No Joker Card')
      }
    )
  } else {
    if (joker > 0) {
...
function printCardName() {
  ...
  for (var j = 0; j < cards.length; j++) {
    ...
    str =
      str +
      `${alphabet.charAt(k++)}. ${
        cardName.length == 2 ? ' ' + cardName : cardName
      } ${i < 6 ? '    ' : ''}`
    if (i >= 6) {
      console.log(str)
      i = 0
...
function getCardFullName(card) {
  var cardName = card
    .replaceAll('13', 'King of ')
    .replaceAll('12', 'Queen of ')
    .replaceAll('11', 'Jack of ')
    .replaceAll('10', 'Ten of ')
  cardName = cardName
    .replaceAll('1', 'Ace of ')
    .replaceAll('2', 'Two of ')
    .replaceAll('3', 'Three of ')
  cardName = cardName
    .replaceAll('4', 'Four of ')
    .replaceAll('5', 'Five of ')
    .replaceAll('6', 'Six of ')
  cardName = cardName
    .replaceAll('7', 'Seven of ')
    .replaceAll('8', 'Eight of ')
    .replaceAll('9', 'Nine of ')
  if (cardName.charAt(cardName.length - 1) == 's')
    cardName = cardName.substring(0, cardName.length - 1) + 'Spade'
  else if (cardName.charAt(cardName.length - 1) == 'h')
    cardName = cardName.substring(0, cardName.length - 1) + 'Heart'
  else if (cardName.charAt(cardName.length - 1) == 'd')
    cardName = cardName.substring(0, cardName.length - 1) + 'Diamond'
  else if (cardName.charAt(cardName.length - 1) == 'c')
    cardName = cardName.substring(0, cardName.length - 1) + 'Club'
...
function hasValidCard(top) {
  for (var card of cards) if (isValidCard(card, top)) return true
  return false
}
...
function compareFunction(a, b) {
  ...
  var numA = parseInt(a.substring(0, a.length - 1))
  var charA = a.substring(a.length - 1)
  var numB = parseInt(b.substring(0, b.length - 1))
  var charB = b.substring(b.length - 1)
